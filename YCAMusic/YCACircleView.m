...
- (void)drawRect:(CGRect)rect {
    ...
    // 画边框圆
    CGContextAddArc(ctr, self.frame.size.width / 2, self.frame.size.height / 2, self.radious, AngleToRadian(STARTANGLE), AngleToRadian(ENDANGLE), 1);
    ...
    CGFloat endAngle = -STARTANGLE + (self.currentTime * MARGINANGLE / self.totalTime);
    ...
    float x = self.frame.size.width / 2 + cos(radian)*self.radious;
    float y = self.frame.size.height / 2 + sin(radian)*self.radious;
